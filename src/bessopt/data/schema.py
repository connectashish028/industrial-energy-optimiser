...
SRC_REGELLEISTUNG = "regelleistung"
...
def _build_columns() -> list[Column]:
    ...
    # 6. Reserve-market capacity prices (regelleistung.net), EUR/MW/h, 4h blocks.
    #    Representative unless the live feed is enabled — see sources/regelleistung.py.
    RESERVE = [
        ("reserve_fcr__capacity_price",           "FCR capacity price (symmetric), EUR/MW/h"),
        ("reserve_afrr_cap__pos__capacity_price", "aFRR POS capacity price, EUR/MW/h"),
        ("reserve_afrr_cap__neg__capacity_price", "aFRR NEG capacity price, EUR/MW/h"),
    ]
    for name, desc in RESERVE:
        cols.append(Column(name=name, source=SRC_REGELLEISTUNG, description=desc))
