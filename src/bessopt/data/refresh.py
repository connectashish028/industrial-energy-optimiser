...
for the requested window, and writes/updates `data/de_lu_15min.parquet`.

Usage
-----
    python -m bessopt.data.refresh                         # extend through "now"
    python -m bessopt.data.refresh --through 2026-05-04    # extend through a date
    python -m bessopt.data.refresh --rebuild --start 2022-01-01 --through 2026-05-04
                                                            # full rebuild
...
SOURCES = (
    ...
    SRC_OPEN_METEO, SRC_ENTSOE, SRC_REGELLEISTUNG,
)
...
def _load_source(source_name: str):
    ...
    if source_name == SRC_OPEN_METEO:
        ...
        return open_meteo
    if source_name == SRC_REGELLEISTUNG:
        from .sources import regelleistung
        return regelleistung
